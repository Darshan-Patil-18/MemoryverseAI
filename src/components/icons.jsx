export function GoogleIcon({ className = 'w-5 h-5' }) {
  return (
    <svg className={className} viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg">
      <path fill="#FFC107" d="M43.6 20.5H42V20H24v8h11.3c-1.6 4.6-6 8-11.3 8-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.8 1.1 8 3l5.7-5.7C34.6 6 29.6 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.7-.4-3.5z"/>
      <path fill="#FF3D00" d="M6.3 14.7l6.6 4.8C14.6 15.9 18.9 13 24 13c3.1 0 5.8 1.1 8 3l5.7-5.7C34.6 6 29.6 4 24 4 16.3 4 9.6 8.3 6.3 14.7z"/>
      <path fill="#4CAF50" d="M24 44c5.5 0 10.4-1.9 14.3-5.1l-6.6-5.6C29.6 35 26.9 36 24 36c-5.3 0-9.6-3.4-11.3-8.1l-6.5 5C9.5 39.6 16.2 44 24 44z"/>
      <path fill="#1976D2" d="M43.6 20.5H42V20H24v8h11.3c-.8 2.3-2.3 4.3-4.2 5.7l6.6 5.6C41.8 36 44 30.5 44 24c0-1.3-.1-2.7-.4-3.5z"/>
    </svg>
  )
}

export function SparkChatIcon({ className = 'w-6 h-6' }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        d="M12 3.5c.35 0 .66.23.76.57l1.1 3.6 3.6 1.1a.8.8 0 0 1 0 1.53l-3.6 1.1-1.1 3.6a.8.8 0 0 1-1.53 0l-1.1-3.6-3.6-1.1a.8.8 0 0 1 0-1.53l3.6-1.1 1.1-3.6c.1-.34.4-.57.77-.57Z"
        fill="currentColor"
      />
      <path
        d="M19 14.2c.24 0 .45.16.52.4l.4 1.32 1.32.4a.55.55 0 0 1 0 1.05l-1.32.4-.4 1.32a.55.55 0 0 1-1.05 0l-.4-1.32-1.32-.4a.55.55 0 0 1 0-1.05l1.32-.4.4-1.32c.07-.24.28-.4.53-.4Z"
        fill="currentColor"
        opacity="0.7"
      />
    </svg>
  )
}

export function MarkLogo({ className = 'w-8 h-8' }) {
  return (
    <svg className={className} viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect width="32" height="32" rx="7" fill="#0B1120" />
      <circle cx="10" cy="22" r="2.4" fill="#3FA796" />
      <circle cx="16" cy="10" r="2.4" fill="#D4A24C" />
      <circle cx="23" cy="18" r="2.4" fill="#5FC3B0" />
      <path d="M10 22 L16 10 L23 18" stroke="#D4A24C" strokeWidth="1.4" opacity="0.7" />
    </svg>
  )
}
